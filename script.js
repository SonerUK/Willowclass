...
const wordLists = {
    ...
    function loadNextWordFromSelection() {
        ...
        hideCorrectSpelling(); // Gizlemeyi burada yapalım
        if (wordInputContainer) wordInputContainer.innerHTML = ''; // Önceki inputları temizle
        ...
        if (currentWordData) {
            ...
            playAudioSequence(() => { // Ses bittikten veya hata verdikten sonra çalışacak callback
                // currentWordData'nın hala geçerli olduğunu kontrol etmek iyi olabilir
                 if (!currentWordData) {
                     console.error("Callback after audio, but currentWordData became null!");
                     // IDLE duruma geri dön
                ...
                listenAgainButton.disabled = false; // Ensure listen again is enabled
                displayNewWordUI(); // UI'ı giriş için hazırla
            });
        } else {
            // Oyun bitti veya kelime bulunamadı durumu
             if (feedbackDiv) {
                 if (activeWordList.length > 0 && playedWordsFromActiveList.length === activeWordList.length) {
                    feedbackDiv.textContent = "Congratulations! You finished all words in the selection.";
                    feedbackDiv.className = 'correct';
                 } else if (activeWordList.length === 0 && playedWordsFromActiveList.length === 0){
                    // Bu durum buildActiveWordList'te zaten ele alınmalı
                 } else {
                     feedbackDiv.textContent = "No more words available.";
                     feedbackDiv.className = '';
                 }
             }
            currentGameState = GAME_STATE.IDLE;
    ...
    function playAudioSequence(callbackOnEnd) {
        if (!currentWordData || !currentWordData.audioPath) {
            if (feedbackDiv && currentWordData) {
                feedbackDiv.textContent = `Audio file missing for "${currentWordData.word}".`;
             ...
             if (mainActionButton) {
                mainActionButton.textContent = "Check Spelling"; // Allow user to try spelling
                mainActionButton.disabled = false;
             }
        ...
        audioPlayer.onerror = (e) => {
            ...
            listenAgainButton.disabled = false;
            if (feedbackDiv) {
                feedbackDiv.textContent = "Error playing sound file.";
                feedbackDiv.className = 'incorrect';
            }
            // Ensure controls are usable on error
             if(mainActionButton) {
                mainActionButton.textContent = "Check Spelling";
                mainActionButton.disabled = false;
             }
        ...
        // console.log("Attempting to play:", currentWordData.audioPath);
        audioPlayer.play().then(() => {
            ...
            if (feedbackDiv) {
                 feedbackDiv.textContent = "Audio playback failed. Please try again.";
                 feedbackDiv.className = 'incorrect';
            }
             displayNewWordUI(); // Show inputs anyway
            if (callbackOnEnd) callbackOnEnd();
    ...
    function displayCorrectSpelling(word) {
        ...
        correctWordLabelElement.classList.remove('hidden'); // Use classList
        correctWordFeedbackArea.classList.remove('hidden'); // Use classList
    }

    function hideCorrectSpelling() {
        if (correctWordFeedbackArea) correctWordFeedbackArea.classList.add('hidden'); // Use classList
        if (correctWordLabelElement) correctWordLabelElement.classList.add('hidden'); // Use classList
    ...
    function checkWord() {
        ...
        if (!allBoxesFilled) {
            ...
            return; // User should continue filling
        }

        currentGameState = GAME_STATE.CHECKED;
        mainActionButton.textContent = "Next Word";
        mainActionButton.disabled = false; // Enable Next Word
        listenAgainButton.disabled = true; // Disable listen again after check
        ...
        if (allCorrect) {
            ...
            letterInputs.forEach(input => input.disabled = true); // Disable inputs on correct
             hideCorrectSpelling(); // Hide if it was shown before for some reason
        } else {
            if (feedbackDiv) {
            ...
            if (firstIncorrectInput) {
               // firstIncorrectInput.focus(); // No need to focus if user will click "Next Word"
            }
        }
    ...
    function updateRemainingCount() {
        ...
        // If the game just finished (pickNewWord returned null and set currentWordData to null)
        if (!currentWordData && totalInActiveList > 0 && playedCount === totalInActiveList) {
            remaining = 0;
        }
        // If a word is currently active, remaining calculation is correct.
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (missingElement || categoryCheckboxes.length !== 4) {
            const missingId = criticalElements.find(el => !el)?.id || (categoryCheckboxes.length !== 4 ? 'category checkboxes (expected 4)' : 'unknown element');
            console.error(`Error: Essential page element missing or incorrect count: ${missingId}. Aborting script.`);
            const container = document.querySelector('.container') || document.body;
            if (container) {
                const errorMsgElement = document.createElement('p');
                 errorMsgElement.className = 'startup-error-message';
                 errorMsgElement.textContent = `Error: Essential page element '${missingId}' missing or incorrect setup. Please check the HTML structure and IDs carefully.`;
        ...
        // Check if wordLists is populated
        if (!wordLists || Object.keys(wordLists).every(key => wordLists[key].length === 0)) {
            ...
            if (remainingCountSpan) remainingCountSpan.textContent = 0;
             ...
             updateRemainingCount(); // Initial count (0)
        }
        ...
        // updateRemainingCount(); // Called initially after check/on change
    });
