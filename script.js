...
const wordLists = {
    ...
    function pickNewWordFromActiveList() {
        ...
        if (availableWords.length === 0) {
            if (activeWordList.length > 0) {
                // console.log("Restarting list from selection...");
                playedWordsFromActiveList = [];
                availableWords = [...activeWordList];
            } else {
                 // console.log("pickNewWordFromActiveList: Active list is empty even after reset attempt.");
                return null;
            }
        }
        if (availableWords.length === 0) {
            // console.log("pickNewWordFromActiveList: No available words found.");
            return null;
        }
    ...
    function loadNextWordFromSelection() {
        ...
        if (currentWordData) {
            ...
            playAudioSequence(() => {
                 if (!currentWordData) { // Check if word data became null (e.g. list exhausted during audio error?)
            ...
            // Handle game over state more robustly
             if (feedbackDiv) {
                 if (activeWordList.length > 0 && playedWordsFromActiveList.length === activeWordList.length) {
                    ...
                    feedbackDiv.className = 'correct';
                     ...
                     feedbackDiv.textContent = "Selected categories appear to be empty.";
                      feedbackDiv.className = '';
                 } else { // Fallback / Should not normally happen if pickNewWord logic is sound
    ...
    function playAudioSequence(callbackOnEnd) {
        if (!currentWordData || !currentWordData.audioPath) {
            if (feedbackDiv && currentWordData) {
             ...
             // Ensure controls become usable if audio is missing
             if(mainActionButton) {
                mainActionButton.textContent = "Check Spelling";
                mainActionButton.disabled = false;
             }
             ...
             displayNewWordUI();
            if (callbackOnEnd) callbackOnEnd(); // Trigger callback even if audio missing
            return;
        }
        if (isAudioPlaying) {
            audioPlayer.pause();
        ...
        // Prepend './' if path doesn't start with http or / maybe? No, relative paths should work.
        ...
        audioPlayer.onerror = (e) => {
            ...
            listenAgainButton.disabled = false;
            if (feedbackDiv) {
                feedbackDiv.textContent = `Error playing sound: ${currentWordData.audioPath}. Check path/file.`;
                feedbackDiv.className = 'incorrect';
            }
             if(mainActionButton) {
                mainActionButton.textContent = "Check Spelling";
        ...
        audioPlayer.play().then(() => {
            ...
            if (feedbackDiv) {
                 feedbackDiv.textContent = "Audio playback failed. Browser might have blocked it or file path incorrect.";
                 feedbackDiv.className = 'incorrect';
            }
             displayNewWordUI(); // Show inputs anyway
            if (callbackOnEnd) callbackOnEnd();
    ...
    function displayNewWordUI() {
        // hideCorrectSpelling() is called by loadNextWordFromSelection now
        createLetterInputs();
    ...
    function createLetterInputs() {
        ...
        for (let i = 0; i < word.length; i++) {
            ...
            // Set input size attribute for better mobile keyboard rendering if needed (optional)
            input.setAttribute('size', '1');

            input.addEventListener('input', (e) => {
                const value = e.target.value;
                if (value.length > 0) {
                    e.target.value = value.toUpperCase()[0]; // Keep single uppercase char
                    const nextInput = wordInputContainer.children[i + 1];
                    if (nextInput) nextInput.focus();
                }
                // Remove previous validation styling when user types
            ...
            input.addEventListener('keydown', (e) => {
                if (e.key === 'Backspace') {
                    ...
                    if (!e.target.value && i > 0) { // If box is empty and not the first one
                        e.preventDefault(); // Prevent browser back navigation
                        const prevInput = wordInputContainer.children[i - 1];
                        prevInput.focus();
                        prevInput.value = ''; // Clear previous box too
                        prevInput.classList.remove('correct', 'incorrect');
                    }
                    // If box has value, default backspace will clear it, then on next backspace the above condition hits.
                    ...
                    e.preventDefault();
                    wordInputContainer.children[i - 1].focus();
                } else if (e.key.length === 1 && e.key.match(/[a-z0-9]/i)) { // Allow letters/numbers
                    // If user types a char in a filled box, allow overwriting
                    e.target.classList.remove('correct', 'incorrect');
                    // The 'input' event will handle capitalization and moving next
                } else if (e.key === 'Enter') {
                    e.preventDefault();
    ...
    function checkWord() {
        ...
        for (let i = 0; i < targetWord.length; i++) {
            ...
            if (!input || !input.value) { // Extra check for input existence
                 allBoxesFilled = false;
                 if(input) input.classList.remove('correct', 'incorrect'); // Clear styles if empty
            } else {
        ...
        if (allCorrect) {
            ...
            letterInputs.forEach(input => input.disabled = true);
             hideCorrectSpelling(); // Hide correct spelling area if it was somehow visible
        } else {
            if (feedbackDiv) {
            ...
            if (firstIncorrectInput) {
               // firstIncorrectInput.focus(); // Optional focus
            }
        }
    ...
    function updateRemainingCount() {
        ...
        // If game is over (no current word, but list had words)
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (missingElement || !categoryCheckboxesOk) {
            const missingId = missingElement ? Object.keys(criticalElements).find(key => criticalElements[key] === missingElement) : (!categoryCheckboxesOk ? 'category checkboxes (expected 4)' : 'unknown element');
            ...
            if (container) {
                const errorMsgElement = document.createElement('p');
                 ...
                 errorMsgElement.textContent = `Error: Essential page element '${missingId}' missing or incorrect setup. Please check the HTML structure and element IDs carefully.`;
        ...
        listenAgainButton.addEventListener('click', () => {
            if (currentWordData && !isAudioPlaying) { // Check if audio is not already playing
