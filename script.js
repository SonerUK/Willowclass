...
// Kelime Listeleri - TAM DOLDURULMUŞ (Bir önceki yanıttaki gibi)
const wordLists = {
...
// let correctCount = 0; // Kaldırıldı
let wordsToPlay = [];
let playedWordsFromActiveList = [];
let activeWordList = []; // <<<---- GLOBAL TANIMLAMA BURADA ----<<<

// DOM Element Referansları
let correctWordFeedbackArea, correctWordDisplayContainer, correctWordLabelElement;
let mainActionButton, listenAgainButton, feedbackDiv; // Kaldırılan referanslar: wordInputContainer, scoreArea, correctCountSpan, remainingCountSpan
...
const GAME_STATE = {
    ...
    AWAITING_REVEAL: 'awaiting_reveal', // State adı güncellendi
    REVEALED: 'revealed' // State adı güncellendi
};
...
function handleMainActionButtonClick() {
    ...
    switch (currentGameState) {
        case GAME_STATE.IDLE:
            startGame();
            break;
        case GAME_STATE.AWAITING_REVEAL: // Ses bittikten sonraki durum
            revealWord(); // Kelimeyi göster
            break;
        case GAME_STATE.REVEALED: // Kelime gösterildikten sonraki durum
            loadNextWordFromSelection(); // Sonraki kelimeye geç
            break;
...
function startGame() {
    ...
    // scoreArea.classList.add('hidden'); // Kaldırıldı
...
function buildActiveWordList(categories) {
    ...
    // updateRemainingCount(); // Kaldırıldı
}
...
function loadNextWordFromSelection() {
    ...
    if (currentWordData) {
        ...
        playAudioSequence(() => {
            ...
            currentGameState = GAME_STATE.AWAITING_REVEAL; // Durumu güncelle
            mainActionButton.textContent = "Show Word"; // Buton metnini değiştir
            ...
            // displayNewWordUI(); // REMOVED - Input gösterilmiyor
        });
    } else {
         if (feedbackDiv) {
             ...
             } else { // Kategoriler boşsa veya başka bir sorun varsa
...
function playAudioSequence(callbackOnEnd) {
    ...
    if (!currentWordData || !currentWordData.audioPath) {
        ...
        if(mainActionButton) { mainActionButton.textContent = "Show Word"; mainActionButton.disabled = false; } // Hata olsa bile kelimeyi göstermeye izin ver
...
// REMOVED: displayNewWordUI function
...
// Kelimeyi Gösterme Fonksiyonu (checkWord yerine)
function revealWord() {
    ...
    if(feedbackDiv) feedbackDiv.textContent = ""; // Geri bildirimleri temizle
}

// REMOVED: checkWord function

// REMOVED: updateRemainingCount function
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // DOM Elementlerini ata (Gereksizler kaldırıldı)
    ...
    // scoreArea = document.getElementById('score-area'); // Kaldırıldı
    ...
    // activeWordList global değişkeninin tanımlı olup olmadığını KONTROL ET
    if (typeof activeWordList === 'undefined') {
         console.error("CRITICAL ERROR: Global variable 'activeWordList' was not defined!");
         // Hata mesajını sayfada göster
          const container = document.querySelector('.container') || document.body;
         if (container) {
             const errorMsgElement = document.createElement('p');
             errorMsgElement.className = 'startup-error-message';
             errorMsgElement.textContent = "CRITICAL SCRIPT ERROR: 'activeWordList' is missing. Ensure 'let activeWordList = [];' exists globally.";
             errorMsgElement.style.color = "red"; errorMsgElement.style.fontWeight = "bold";
             const existingError = container.querySelector('.startup-error-message');
             if(existingError) existingError.remove();
             container.insertBefore(errorMsgElement, container.firstChild);
    ...
    categoryCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', () => {
            ...
            listenAgainButton.classList.add('hidden');
            // wordInputContainer referansı ve temizliği kaldırıldı
            hideCorrectSpelling();
            if(feedbackDiv) feedbackDiv.textContent = "";
