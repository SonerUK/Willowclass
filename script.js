// Kategoriye göre kelime listeleri - TAM DOLDURULMUŞ
const wordLists = {
...
let correctCount = 0; // Removed as score is not needed now
let wordsToPlay = [];
let playedWordsFromActiveList = [];
let activeWordList = []; // <<< DECLARED GLOBALLY HERE

// DOM Elements
let wordInputContainer, correctWordFeedbackArea, correctWordDisplayContainer, correctWordLabelElement;
let mainActionButton, listenAgainButton, feedbackDiv, scoreArea; // Removed count spans
...
const GAME_STATE = {
    ...
    AWAITING_REVEAL: 'awaiting_reveal', // Renamed state
    REVEALED: 'revealed'        // Renamed state
};
...
function handleMainActionButtonClick() {
    ...
    switch (currentGameState) {
        case GAME_STATE.IDLE:
            startGame();
            break;
        case GAME_STATE.AWAITING_REVEAL: // State after audio finishes
            revealWord();
            break;
        case GAME_STATE.REVEALED: // State after word is shown
...
function startGame() {
    ...
    // scoreArea.classList.add('hidden'); // Hide score area as it's removed
...
function buildActiveWordList(categories) {
    ...
    // updateRemainingCount(); // No remaining count needed now
}
...
function loadNextWordFromSelection() {
    ...
    hideCorrectSpelling(); // Hide previous correct spelling
    // No need to clear wordInputContainer as it doesn't exist
    ...
    if (currentWordData) {
        ...
        playAudioSequence(() => { // Callback runs after audio finishes or errors
             if (!currentWordData) { // Safety check
                 console.error("Audio callback error: currentWordData is null!");
                 currentGameState = GAME_STATE.IDLE; mainActionButton.textContent = "Start"; mainActionButton.disabled = false;
                 return;
             }
            currentGameState = GAME_STATE.AWAITING_REVEAL; // Set state to wait for reveal
            mainActionButton.textContent = "Show Word"; // Change button text
            mainActionButton.disabled = false; // Enable button
            listenAgainButton.classList.remove('hidden'); // Show listen again button
            listenAgainButton.disabled = false;
            // displayNewWordUI(); // REMOVED - No inputs to show
        });
    } else {
    ...
    // updateRemainingCount(); // No remaining count needed
}
...
function playAudioSequence(callbackOnEnd) {
    ...
    if (!currentWordData || !currentWordData.audioPath) {
        ...
        // Ensure UI becomes usable
        if(mainActionButton) { mainActionButton.textContent = "Show Word"; mainActionButton.disabled = false; } // Allow user to try revealing word anyway
        if(listenAgainButton) listenAgainButton.classList.remove('hidden');
        // displayNewWordUI(); // REMOVED
        if (callbackOnEnd) setTimeout(callbackOnEnd, 0);
        return;
    }
    if (isAudioPlaying) { audioPlayer.pause(); audioPlayer.currentTime = 0; }
    isAudioPlaying = true;
    mainActionButton.disabled = true; listenAgainButton.disabled = true;
    audioPlayer.src = currentWordData.audioPath;
    const onEnd = () => { cleanupAudio(); if (callbackOnEnd) callbackOnEnd(); };
    const onError = (e) => { console.error("Error playing audio:", currentWordData.audioPath, e); cleanupAudio(); if (feedbackDiv) { feedbackDiv.textContent = `Error playing sound. Check path/file & console (F12).`; feedbackDiv.className = 'incorrect'; } if(mainActionButton) { mainActionButton.textContent = "Show Word"; mainActionButton.disabled = false; } /* displayNewWordUI(); REMOVED */ if (callbackOnEnd) callbackOnEnd(); };
    ...
    audioPlayer.play().catch(e => { console.error("Audio play() rejected:", e); cleanupAudio(); if (feedbackDiv) { feedbackDiv.textContent = "Audio playback failed. Check console (F12)."; feedbackDiv.className = 'incorrect'; } if (mainActionButton) mainActionButton.disabled = false; /* displayNewWordUI(); REMOVED */ if (callbackOnEnd) callbackOnEnd(); });
}

// REMOVED: displayNewWordUI function as inputs are not shown
// REMOVED: createLetterInputs function

// Doğru Kelimeyi Gösterme (Bu fonksiyon kalıyor)
function displayCorrectSpelling(word) {
    ...
    correctWordLabelElement.classList.remove('hidden'); // Label'ı göster
    correctWordFeedbackArea.classList.remove('hidden'); // Alanı göster
}

// Doğru Kelime Gösterimini Gizleme (Bu fonksiyon kalıyor)
...
// YENİ: Kelimeyi Gösterme Fonksiyonu (checkWord yerine)
function revealWord() {
    // console.log("revealWord called");
    if (!currentWordData) return; // Kelime yoksa bir şey yapma

    displayCorrectSpelling(currentWordData.word); // Doğru kelimeyi göster
    currentGameState = GAME_STATE.REVEALED; // Durumu güncelle
    mainActionButton.textContent = "Next Word"; // Butonu güncelle
    mainActionButton.disabled = false; // Butonu aktif et
    listenAgainButton.disabled = true; // Kelime gösterildikten sonra tekrar dinlemeyi kapat
    if(feedbackDiv) feedbackDiv.textContent = ""; // Önceki geri bildirimleri temizle
}

// REMOVED: checkWord function

// REMOVED: updateRemainingCount function (Artık skor veya kalan kelime sayımı yok)
...
document.addEventListener('DOMContentLoaded', () => {
    // DOM Elementlerini ata (Skor ile ilgili olanlar kaldırıldı)
    // wordInputContainer referansı da kaldırıldı
    ...
    // correctCountSpan = document.getElementById('correct-count'); // REMOVED
    // remainingCountSpan = document.getElementById('remaining-count'); // REMOVED
    // scoreArea = document.getElementById('score-area'); // REMOVED
    categoryCheckboxes = document.querySelectorAll('input[name="category"]');

    // Kritik elementler var mı kontrol et (Güncellenmiş liste)
    ...
    listenAgainButton.addEventListener('click', () => {
        if (currentWordData && !isAudioPlaying) {
             playAudioSequence(() => {}); // Sadece sesi tekrar çal
        }
    });
    ...
    categoryCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', () => {
            ...
            mainActionButton.disabled = false;
            listenAgainButton.classList.add('hidden');
            // wordInputContainer temizleme kaldırıldı
            hideCorrectSpelling();
            if(feedbackDiv) feedbackDiv.textContent = "";
            currentWordData = null;
            // scoreArea gizleme kaldırıldı
            activeWordList = [];
